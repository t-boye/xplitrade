...
function updateenv() {
    ...
    ${PIP} install -e .
    if [ $? -ne 0 ]; then
        echo "Failed installing Xplitrade"
        exit 1
    fi
...
function config() {
    echo_block "Please use 'xplitrade new-config -c user_data/config.json' to generate a new configuration file."
}
...
function install() {
    ...
    echo "You can now use the bot by executing 'source .venv/bin/activate; xplitrade <subcommand>'."
    echo "You can see the list of available bot sub-commands by executing 'source .venv/bin/activate; xplitrade --help'."
    echo "You verify that xplitrade is installed successfully by running 'source .venv/bin/activate; xplitrade --version'."
}
...
function help() {
    echo "usage:"
    echo "	-i,--install    Install xplitrade from scratch"
