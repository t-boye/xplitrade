...
This page provides you some basic concepts on how Xplitrade works and operates.

## Xplitrade terminology
...
All profit calculations of Xplitrade include fees. For Backtesting / Hyperopt / Dry-run modes, the exchange default fee is used (lowest tier on the exchange). For live operations, fees are used as applied by the exchange (this includes BNB rebates etc.).

## Pair naming

Xplitrade follows the [ccxt naming convention](https://docs.ccxt.com/#/README?id=consistency-of-base-and-quote-currencies) for currencies.
...
Starting xplitrade in dry-run or live mode (using `xplitrade trade`) will start the bot and start the bot iteration loop.
