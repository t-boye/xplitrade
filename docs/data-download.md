...
To download data (candles / OHLCV) needed for backtesting and hyperoptimization use the `xplitrade download-data` command.

If no additional parameter is specified, xplitrade will download data for `"1m"` and `"5m"` timeframes for the last 30 days.
...
!!! Tip "Tip: Updating existing data"
    If you already have backtesting data available in your data-directory and would like to refresh this data up to today, xplitrade will automatically calculate the missing timerange for the existing pairs and the download will occur from the latest available point until "now", neither `--days` or `--timerange` parameters are required. Xplitrade will keep the available data and only download the missing data.  
...
!!! Tip "Downloading all data for one quote currency"
    ...
    `xplitrade download-data --exchange binance --pairs ".*/USDT" <...>`. The provided "pairs" string will be expanded to contain all active pairs on the exchange.
...
```bash
xplitrade download-data --exchange binance --pairs ETH/USDT XRP/USDT BTC/USDT
```

or as regex (in this case, to download all active USDT pairs)

```bash
xplitrade download-data --exchange binance --pairs ".*/USDT"
```
...
* When downloading futures data (`--trading-mode futures` or a configuration specifying futures mode), xplitrade will automatically download the necessary candle types (e.g. `mark` and `funding_rate` candles) unless specified otherwise via `--candle-types`.
...
``` bash
xplitrade download-data --exchange binance --pairs BTC/USDT:USDT --timeframes 1h --trading-mode futures --candle-types open_interest
```
...
``` bash
xplitrade download-data --exchange binance --pairs ETH/USDT XRP/USDT BTC/USDT --prepend --timerange 20210101-20220101
```

!!! Note
    Xplitrade will ignore the end-date in this mode if data is available, updating the end-date to the existing data start point.

### Data format

Xplitrade currently supports the following data-formats:
...
``` bash
time xplitrade list-data --show-timerange --data-format-ohlcv <dataformat>
```
...
The following command will convert all candle (OHLCV) data available in `~/.xplitrade/data/binance` from json to jsongz, saving diskspace in the process.
It'll also remove original json data files (`--erase` parameter).

``` bash
xplitrade convert-data --format-from json --format-to jsongz --datadir ~/.xplitrade/data/binance -t 5m 15m --erase
```
...
The following command will convert all available trade-data in `~/.xplitrade/data/kraken` from jsongz to json.
It'll also remove original jsongz data files (`--erase` parameter).

``` bash
xplitrade convert-trade-data --format-from jsongz --format-to json --datadir ~/.xplitrade/data/kraken --erase
```
...
``` bash
xplitrade trades-to-ohlcv --exchange kraken -t 5m 1h 1d --pairs BTC/EUR ETH/EUR
```
...
```bash
> xplitrade list-data --userdir ~/.xplitrade/user_data/
...
``` bash
> xplitrade list-data --show --trades
...
```bash
xplitrade download-data --exchange kraken --pairs XRP/EUR ETH/EUR --days 20 --dl-trades
```
