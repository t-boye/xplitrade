...
Creates the directory structure to hold your files for xplitrade.
...
!!! Warning
    Only vital questions are asked. Xplitrade offers a lot more configuration possibilities, which are listed in the [Configuration documentation](configuration.md#configuration-parameters)

### Create config examples

```
$ xplitrade new-config --config user_data/config_binance.json
...
```bash
xplitrade new-strategy --userdir ~/.xplitrade/ --strategy AwesomeStrategy
```

Using the advanced template (populates all optional functions and methods)

```bash
xplitrade new-strategy --strategy AwesomeStrategy --template advanced
```
...
``` bash
xplitrade list-strategies
```

Example: Search strategies  directory within the userdir.

``` bash
xplitrade list-strategies --userdir ~/.xplitrade/
```

Example: Search dedicated strategy path.

``` bash
xplitrade list-strategies --strategy-path ~/.xplitrade/strategies/
```
...
```
$ xplitrade list-exchanges
Exchanges available for Xplitrade:
...
Example: see all exchanges supported by the ccxt library (including 'bad' ones, i.e. those that are known to not work with Xplitrade)

```
$ xplitrade list-exchanges -a
...
```
$ xplitrade list-timeframes -c config_binance.json
...
Timeframes available for the exchange `binance`: 1m, 3m, 5m, 15m, 30m, 1h, 2h, 4h, 6h, 8h, 12h, 1d, 3d, 1w, 1M
```

* Example: enumerate exchanges available for Xplitrade and print timeframes supported by each of them:
```
$ for i in `xplitrade list-exchanges -1`; do xplitrade list-timeframes --exchange $i; done
```
...
For pairs traded by Xplitrade the pair quote currency is defined by the value of the `stake_currency` configuration setting.
...
```
$ xplitrade list-pairs --quote USD --print-json
```
...
```
$ xplitrade list-pairs -c config_binance.json --all --base BTC ETH --quote USDT USD --print-list
```

* Print all markets on exchange "Kraken", in the tabular format:

```
$ xplitrade list-markets --exchange kraken --all
```
...
```
xplitrade test-pairlist --config config.json --quote USDT BTC
```

## Convert database

`xplitrade convert-db` can be used to convert your database from one system to another (sqlite -> postgres, postgres -> other postgres), migrating all trades, orders and Pairlocks.
...
!!! Warning
    Please ensure to only use this on an empty target database. Xplitrade will perform a regular migration, but may fail if entries already existed.
...
You can use `docker compose run --rm -p 127.0.0.1:8080:8080 xplitrade webserver` to start a one-off container that'll be removed once you stop it. This assumes that port 8080 is still available and no other bot is running on that port.
...
```
xplitrade hyperopt-list --profitable --no-details
```
...
```
xplitrade hyperopt-show -n 168
```

Prints JSON data with details for the last best epoch (i.e., the best of all epochs):

```
xplitrade hyperopt-show --best -n -1 --print-json --no-header
```
...
``` bash
xplitrade show-trades --db-url sqlite:///tradesv3.sqlite --trade-ids 2 3 --print-json
```
