...
## Running multiple instances of Xplitrade
...
* Use different ports (applies only when Xplitrade REST API webserver is enabled).

### Different database files

In order to keep track of your trades, profits, etc., xplitrade is using a SQLite database where it stores various types of information such as the trades you performed in the past and the current position(s) you are holding at any time. This allows you to keep track of your profits, but most importantly, keep track of ongoing activity if the bot process would be restarted or would be terminated unexpectedly.

Xplitrade will, by default, use separate database files for dry-run and live bots (this assumes no database-url is given in either configuration nor via command line argument).
...
``` bash
xplitrade trade -c MyConfig.json -s MyStrategy
# is equivalent to
xplitrade trade -c MyConfig.json -s MyStrategy --db-url sqlite:///tradesv3.dryrun.sqlite
```
...
If you specify the URL of a database which does not exist, xplitrade will create one with the name you specified. So to test your custom strategy with BTC and USDT stake currencies, you could use the following commands (in 2 separate terminals):

``` bash
# Terminal 1:
xplitrade trade -c MyConfigBTC.json -s MyCustomStrategy --db-url sqlite:///user_data/tradesBTC.dryrun.sqlite
# Terminal 2:
xplitrade trade -c MyConfigUSDT.json -s MyCustomStrategy --db-url sqlite:///user_data/tradesUSDT.dryrun.sqlite
```
...
``` bash
# Terminal 1:
xplitrade trade -c MyConfigBTC.json -s MyCustomStrategy --db-url sqlite:///user_data/tradesBTC.live.sqlite
# Terminal 2:
xplitrade trade -c MyConfigUSDT.json -s MyCustomStrategy --db-url sqlite:///user_data/tradesUSDT.live.sqlite
```
...
To run multiple instances of xplitrade using docker you will need to edit the docker-compose.yml file and add all the instances you want as separate services. Remember, you can separate your configuration into multiple files, so it's a good idea to think about making them modular, then if you need to edit something common to all bots, you can do that in a single config file. 
``` yml
---
version: '3'
services:
  xplitrade1:
    image: xplitradeorg/xplitrade:stable
    # image: xplitradeorg/xplitrade:develop
    # Use plotting image
    # image: xplitradeorg/xplitrade:develop_plot
    # Build step - only needed when additional dependencies are needed
    # build:
    #   context: .
    #   dockerfile: "./docker/Dockerfile.custom"
    restart: always
    container_name: xplitrade1
    volumes:
...
You can use whatever naming convention you want, xplitrade1 and 2 are arbitrary. Note, that you will need to use different database files, port mappings and telegram configurations for each instance, as mentioned above. 

## Use a different database system

Xplitrade is using SQLAlchemy, which supports multiple different database systems. As such, a multitude of database systems should be supported.
Xplitrade does not depend or install any additional database driver. Please refer to the [SQLAlchemy docs](https://docs.sqlalchemy.org/en/14/core/engines.html#database-urls) on installation instructions for the respective database systems.

The following systems have been tested and are known to work with xplitrade:
...
!!! Warning
    By using one of the below database systems, you acknowledge that you know how to manage such a system. The xplitrade team will not provide any support with setup or maintenance (or backups) of the below database systems.
...
Xplitrade will automatically create the tables necessary upon startup.

If you're running different instances of Xplitrade, you must either setup one database per Instance or use different users / schemas for your connections.

### MariaDB / MySQL

Xplitrade supports MariaDB by using SQLAlchemy, which supports multiple different database systems.
...
Copy the `xplitrade.service` file to your systemd user directory (usually `~/.config/systemd/user`) and update `WorkingDirectory` and `ExecStart` to match your setup.

!!! Note
    Certain systems (like Raspbian) don't load service unit files from the user directory. In this case, copy `xplitrade.service` into `/etc/systemd/user/` (requires superuser permissions).
...
For this to be persistent (run when user is logged out) you'll need to enable `linger` for your xplitrade user.

```bash
sudo loginctl enable-linger "$USER"
```

If you run the bot as a service, you can use systemd service manager as a software watchdog monitoring xplitrade bot 
...
The `xplitrade.service.watchdog` file contains an example of the service unit configuration file which uses systemd 
as the watchdog.
...
Xplitrade uses the default logging module provided by python.
...
Default logging format (coloured terminal output) is set up by default if no `log_config` is provided in your xplitrade configuration.
Using `--logfile logfile.log` will enable the RotatingFileHandler.

If you're not content with the log format, or with the default settings provided for the RotatingFileHandler, you can customize logging to your liking by adding the `log_config` configuration to your xplitrade configuration file(s).
...
``` json hl_lines="5-7 13-16 27"
{
  "log_config": {
      ...
      "handlers": {
          "console": {
              "class": "xplitrade.loggers.ft_rich_handler.FtRichHandler",
              "formatter": "basic"
          },
          "file": {
...
If this section is left out, xplitrade will provide no output (in the non-configured handler, anyway).

!!! Tip "Explicit log configuration"
    We recommend to extract the logging configuration from your main xplitrade configuration file, and provide it to your bot via [multiple configuration files](configuration.md#multiple-configuration-files) functionality. This will avoid unnecessary code duplication.
...
To send Xplitrade log messages to a local or remote `syslog` service use the `"log_config"` setup option to configure logging.
...
```
if $programname startswith "xplitrade" then -/var/log/xplitrade.log
```
...
To send Xplitrade log messages to `journald` system service, add the following configuration snippet to your configuration.
...
* `journalctl -f` -- shows Xplitrade log messages sent to `journald` along with other log messages fetched by `journald`.
* `journalctl -f -u xplitrade.service` -- this command can be used when the bot is run as a `systemd` service.
...
??? Info "Deprecated - configure journald via command line"
    To send Xplitrade log messages to `journald` system service use the `--logfile` command line option with the value in the following format:
...
``` json
{
  ...
  "log_config": {
    ...
    "formatters": {
       "json": {
          "()": "xplitrade.loggers.json_formatter.JsonFormatter",
          "fmt_dict": {
              "timestamp": "asctime",
              "level": "levelname",
              "logger": "name",
