...
- the [Xplitrade strategy 101](strategy-101.md), which provides a quick start to strategy development
- the [Xplitrade bot basics](bot-basics.md), which provides overall info on how the bot operates
...
```bash
xplitrade new-strategy --strategy AwesomeStrategy
```
...
!!! Note
    There is a difference between the *name* of the strategy and the filename. In most commands, Xplitrade uses the *name* of the strategy, *not the filename*.
...
??? Hint "Different template levels"
    `xplitrade new-strategy` has an additional parameter, `--template`, which controls the amount of pre-build information you get in the created strategy. Use `--template minimal` to get an empty strategy without any indicator examples, or `--template advanced` to get a template with more complicated features defined.
...
```bash
xplitrade trade --strategy AwesomeStrategy
```

### Bot modes

Xplitrade strategies can be processed by the Xplitrade bot in 5 main modes:
...
??? Hint "Lookahead and recursive analysis"
    Xplitrade includes two helpful commands to help assess common lookahead (using future data) and
...
Xplitrade uses [pandas](https://pandas.pydata.org/) to store/provide the candlestick (OHLCV) data.
...
!!! Warning "Row order matters"
    Please do not sort, shuffle, or any other way to change the row order of the dataframe - doing so will make the bot act on the wrong candle. Xplitrade assume last row to be the latest closed candle, hence all actions will be based of the last row.
...
Xplitrade uses this new column as an entry signal, where it is assumed that a trade will subsequently open on the next open candle.
...
Xplitrade does not store incomplete/unfinished candles in the dataframe.
...
Xplitrade does not. Only complete/finished candle data is available in the dataframe.
...
Out of the box, xplitrade installs the following technical libraries:
...
Some indicators have an unstable startup period in which there isn't enough candle data to calculate any values (NaN), or the calculation is incorrect. This can lead to inconsistencies, since Xplitrade does not know how long this unstable period is and uses whatever indicator values are in the dataframe.
...
!!! Warning "Using x calls to get OHLCV"
    ...
    Having this will cause Xplitrade to make multiple calls for the same pair, which will obviously be slower than one network request.
    As a consequence, Xplitrade will take longer to refresh candles - and should therefore be avoided if possible.
    This is capped to 5 total calls to avoid overloading the exchange, or make xplitrade too slow.
...
``` bash
xplitrade backtesting --timerange 20190101-20190201 --timeframe 5m
```
...
``` python
from xplitrade.exchange import timeframe_to_minutes
...
By default, xplitrade will attempt to load strategies from all `.py` files within the `userdir` (default `user_data/strategies`).

Assuming your strategy is called `AwesomeStrategy`, stored in the file `user_data/strategies/AwesomeStrategy.py`, then you can start xplitrade in dry (or live, depending on your configuration) mode with:
...
You can use `xplitrade list-strategies` to see a list of all strategies Xplitrade is able to load (all strategies in the correct folder).
...
be returned by informative callbacks or column formatters, or exist in the base dataframe before informative merging. Xplitrade will raise an exception if this is violated to avoid unexpected merge behavior.
...
??? Example "Fast and easy way to define informative pairs"
    ...
    from xplitrade.strategy import IStrategy, informative
...
This method is used by xplitrade internally to determine the last signal.
...
``` python
from xplitrade.strategy import merge_informative_pair
...
```python
from xplitrade.strategy import IStrategy, merge_informative_pair
...
Xplitrade locks pairs automatically for the current candle (until that candle is over) when a pair exits, preventing an immediate re-entry of that pair.
...
Xplitrade has an easy method to do this from within the strategy, by calling `self.lock_pair(pair, until, [reason])`.
...
When conflicting signals collide (e.g. both `'enter_long'` and `'exit_long'` are set to `1`), xplitrade will do nothing and ignore the entry signal. This will avoid trades that enter, and exit immediately. Obviously, this can potentially lead to missed entries.
