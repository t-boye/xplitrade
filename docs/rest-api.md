...
We advise consuming the API by using the supported `xplitrade-client` package (also available as `scripts/rest_client.py`).

This command can be installed independent of any running xplitrade bot by using `pip install xplitrade-client`.

This module is designed to be lightweight, and only depends on the `requests` and `python-rapidjson` modules, skipping all heavy dependencies xplitrade otherwise needs.

``` bash
xplitrade-client <command> [optional parameters]
```
...
``` bash
xplitrade-client --config rest_config.json <command> [optional parameters]
```

Commands with many arguments may require keyword arguments (for clarity) - which can be provided as follows:

``` bash
xplitrade-client --config rest_config.json forceenter BTC/USDT long enter_tag=GutFeeling
```

This method will work for all arguments - check the "show" command for a list of available parameters.

??? Note "Programmatic use"
    The `xplitrade-client` package (installable independent of xplitrade) can be used in your own scripts to interact with the xplitrade API.
    to do so, please use the following:

    ``` python
    from xplitrade_client import FtRestClient
...
#### Xplitrade client- available commands

Possible commands can be listed from the rest-client script using the `help` command.

``` bash
xplitrade-client help
```

--8<-- "commands/xplitrade-client.md"
...
The API Server includes a websocket endpoint for subscribing to RPC messages from the xplitrade Bot.
...
This is also used to setup [Producer/Consumer mode](producer-consumer.md) in Xplitrade.
...
For a list of message types, please refer to the RPCMessageType enum in `xplitrade/enums/rpcmessagetype.py`
...
Please make sure to replace `<xplitrade_listen_ip>` (and the subsequent port) with the IP and Port matching your configuration/setup.
...
http {
    ...
    server {
        ...
        location / {
            ...
            proxy_pass http://<xplitrade_listen_ip>:8080;
...
!!! Tip "SSL certificates"
    ...
    While this will protect your data in transit, we do not recommend to run the xplitrade API outside of your private network (VPN, SSH tunnel).
...
!!! Note
    The below should be done in an application (a Xplitrade REST API client, which fetches info via API), and is not intended to be used on a regular basis.

Xplitrade's REST API also offers JWT (JSON Web Tokens).
You can login using the following command, and subsequently use the resulting access_token.

``` bash
> curl -X POST --user Xplitrader http://localhost:8080/api/v1/token/login
