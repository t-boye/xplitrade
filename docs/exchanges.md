...
Xplitrade is based on [CCXT library](https://github.com/ccxt/ccxt) that supports over 100 cryptocurrency
...
If not enough `BNB` is available to cover transaction fees, then fees will not be covered by `BNB` and no fee reduction will occur. Xplitrade will never buy BNB to cover for fees. BNB needs to be bought and monitored manually to this end.
...
Xplitrade supports binance RSA API keys.
...
These settings will be checked on startup, and xplitrade will show an error if this setting is wrong.

![Binance futures settings](assets/binance_futures_settings.png)

Xplitrade will not attempt to change these settings.
...
Xplitrade will check these settings on startup, but won't attempt to change them.
...
The Kraken API does only provide 720 historic candles, which is sufficient for Xplitrade dry-run and live trade modes, but is a problem for backtesting.
...
These are usually updated once per quarter. Xplitrade expects these files to be placed in `user_data/data/kraken/trades_csv`.
...
You can convert these files into xplitrade files:

``` bash
xplitrade convert-trade-data --exchange kraken --format-from kraken_csv --format-to feather
# Convert trade data to different ohlcv timeframes
xplitrade trades-to-ohlcv -p BTC/EUR BCH/EUR --exchange kraken -t 1m 5m 15m 1h
```

The converted data also makes downloading data possible, and will start the download after the latest loaded trade.

``` bash
xplitrade download-data --exchange kraken --dl-trades -p BTC/EUR BCH/EUR 
```
...
!!! Warning "Downloading data from kraken"
    ...
    It will also take a long time, as xplitrade will need to download every single trade that happened on the exchange for the pair / timerange combination, therefore please be patient.
...
!!! Note "Flex (Multi-collateral) Accounts"
    ...
    Xplitrade derives the `USD` balance from Kraken margin fields, so keep `stake_currency` set to `USD`.
...
!!! Warning "Futures"
    ...
    Xplitrade supports both modes (we recommend to use Buy/Sell mode) - but changing the mode mid-trading is not supported and will lead to exceptions and failures to place trades.
...
!!! Warning "Unified accounts"
    Xplitrade assumes accounts to be dedicated to the bot.
...
On startup, xplitrade will set the position mode to "One-way Mode" for the whole (sub)account. This avoids making this call over and over again (slowing down bot operations), but means that manual changes to this setting may result in exceptions and errors.
...
Bybit uses live markets to simulate your trades (without market impact) - making it work very similar to xplitrade's dry-run mode.  
...
!!! Warning "Unified accounts"
    Hyperliquid unified accounts are supported - though this relies xplitrade's assumption of "owning" the account, and being the only one trading on it (in this case, extended to both spot and futures).
    ...
    Xplitrade will attempt to detect the account type on startup - changing the account type mid-trading is not supported and may lead to exceptions and errors.
...
!!! Info "Some general best practices (non exhaustive)"
    ...
    * Don't store your actual wallet private key on the server you use for xplitrade. Use the API wallet private key instead. This key won't allow withdrawals, only trading.
...
To use subaccounts with Xplitrade, you will need to use the following configuration pattern:
...
Hyperliquid allows you to create vaults. To use vaults with Xplitrade, you will need to use the following configuration pattern:
...
To trade on HIP-3 DEXes with Xplitrade, you need to add them to your configuration using the `hip3_dexes` parameter:
...
!!! Warning "Performance and Rate Limit Impact"
    ...
    * **Additional API Calls**: For each HIP-3 DEX configured, Xplitrade needs to make additional API calls.
...
By default, Xplitrade assumes that incomplete candle is fetched from the exchange and removes the last candle assuming it's the incomplete candle.

Whether your exchange returns incomplete candles or not can be checked using [the helper script](developer.md#incomplete-candles) from the Contributor documentation.

Due to the danger of repainting, Xplitrade does not allow you to use this incomplete candle.
...
### Advanced Xplitrade Exchange configuration
