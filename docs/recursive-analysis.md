...
Why does this matter for Xplitrade? In backtesting, the bot will get full data of the pairs according to the timerange specified. But in a dry/live run, the bot will be limited by the amount of data each exchanges gives.
...
`recursive-analysis` can also be triggered through the REST API when running xplitrade in [webserver mode](utils.md#webserver-mode).
As the analysis can run for a while, it is executed as a background task.
