...
!!! Info "Docker compose install"
    Xplitrade documentation assumes the use of Docker desktop (or the docker compose plugin).  
...
docker compose run --rm xplitrade create-userdir --userdir user_data

# Create configuration - Requires answering interactive questions
docker compose run --rm xplitrade new-config --config user_data/config.json
```

The above snippet creates a new directory called `ft_userdata`, downloads the latest compose file and pulls the xplitrade image.
...
This should list the service `xplitrade` as `running`. If that's not the case, best check the logs (see next point).

#### Docker compose logs

Logs will be written to: `user_data/logs/xplitrade.log`.  
...
#### Updating xplitrade with docker

Updating xplitrade when using `docker` is as simple as running the following 2 commands:
...
All xplitrade arguments will be available by running `docker compose run --rm xplitrade <command> <optional arguments>`.
...
??? Note "Using docker without docker compose"
    ...
    Some xplitrade commands that don't require authentication such as `list-pairs` can be run with "`docker run --rm`" instead.  
    For example `docker run --rm xplitradeorg/xplitrade:stable list-pairs --exchange binance --quote BTC --print-json`.  
...
``` bash
docker compose run --rm xplitrade download-data --pairs ETH/BTC --exchange binance --days 5 -t 1h
```
...
``` bash
docker compose run --rm xplitrade backtesting --config user_data/config.json --strategy SampleStrategy --timerange 20190801-20191001 -i 5m
```
...
Commands `xplitrade plot-profit` and `xplitrade plot-dataframe` ([Documentation](plotting.md)) are available by changing the image to `*_plot` in your `docker-compose.yml` file.
You can then use these commands as follows:

``` bash
docker compose run --rm xplitrade plot-dataframe --strategy AwesomeStrategy -p BTC/ETH --timerange=20180801-20180805
```
...
Xplitrade provides a docker-compose file which starts up a jupyter lab server.
...
Since part of this image is built on your machine, it is recommended to rebuild the image from time to time to keep xplitrade (and dependencies) up-to-date.
...
!!! Warning
    ...
    Best use a linux-VPS for running xplitrade reliably.
