...
!!! Bug
    Hyperopt can crash when used with only 1 CPU Core as found out in [Issue #1133](https://github.com/Xplitrade/xplitrade/issues/1133)
...
!!! Tip "Quickly optimize ROI, stoploss and trailing stoploss"
    ...
    xplitrade hyperopt --hyperopt-loss SharpeHyperOptLossDaily --spaces roi stoploss trailing --strategy MyWorkingStrategy --config config.json -e 100
    ```
...
For every new set of parameters, xplitrade will run first `populate_entry_trend()` followed by `populate_exit_trend()`, and then run the regular backtesting process to simulate trades.
...
!!! Warning
    ...
    Xplitrade will log a warning when the value of an optimized parameter is used during indicator calculation - the results of such an epoch would not correspond to the parameters shown for it. Either use the [`.range` functionality](#optimizing-an-indicator-parameter) or use `--analyze-per-epoch` in this case.

## Optimizing an indicator parameter

Assuming you have a simple strategy in mind - a EMA cross strategy (2 Moving averages crossing) - and you'd like to find the ideal parameters for this strategy.
By default, we assume a stoploss of 5% - and a take-profit (`minimal_roi`) of 10% - which means xplitrade will sell the trade once 10% profit has been reached.
...
from xplitrade.strategy import (BooleanParameter, CategoricalParameter, DecimalParameter, 
                                IStrategy, IntParameter)
import xplitrade.vendor.qtpylib.indicators as qtpylib
...
Xplitrade can also optimize protections. How you optimize protections is up to you, and the following should be considered as example only.
...
`xplitrade hyperopt --hyperopt-loss SharpeHyperOptLossDaily --strategy MyAwesomeStrategy --spaces protection`
...
!!! Note
    ...
    Xplitrade will also automatically change the "--enable-protections" flag if the protection space is selected.
...
* `ShortTradeDurHyperOptLoss` - (default legacy Xplitrade hyperoptimization loss function) - Mostly for short trade duration and avoiding losses.
...
```bash
xplitrade hyperopt --config config.json --hyperopt-loss <hyperoptlossname> --strategy <strategyname> -e 500 --spaces all
```
...
```bash
xplitrade hyperopt --strategy <strategyname> --timerange 20210101-20210201
```
...
If you are optimizing ROI, Xplitrade creates the 'roi' optimization hyperspace for you -- it's the hyperspace of components for the ROI tables. By default, each ROI table generated by the Xplitrade consists of 4 rows (steps). Hyperopt implements adaptive ranges for ROI tables with ranges for values in the ROI steps that depend on the timeframe used. By default the values vary in the following ranges (for some of the most used timeframes, values are rounded to 3 digits after the decimal point):
...
If you have the `generate_roi_table()` and `roi_space()` methods in your custom hyperopt, remove them in order to utilize these adaptive ROI tables and the ROI hyperoptimization space generated by Xplitrade by default.
...
If you are optimizing stoploss values, Xplitrade creates the 'stoploss' optimization hyperspace for you. By default, the stoploss values in that hyperspace vary in the range -0.35...-0.02, which is sufficient in most cases.

If you have the `stoploss_space()` method in your custom hyperopt file, remove it in order to utilize Stoploss hyperoptimization space generated by Xplitrade by default.
...
If you are optimizing trailing stop values, Xplitrade creates the 'trailing' optimization hyperspace for you. By default, the `trailing_stop` parameter is always set to True in that hyperspace, the value of the `trailing_only_offset_is_reached` vary between True and False, the values of the `trailing_stop_positive` and `trailing_stop_positive_offset` parameters vary in the ranges 0.02...0.35 and 0.01...0.1 correspondingly, which is sufficient in most cases.
...
By default, hyperopt emulates the behavior of the Xplitrade Live Run/Dry Run, where only one
...
Xplitrade tries to counter the "local minima" problem by using new, randomized points in this case.
...
If you want to output debug messages from your strategy, you can use the `logging` module. By default, Xplitrade will output all messages with a level of `INFO` or higher. 
...
* You may have added parameters to hyperopt in `populate_indicators()` where they will be calculated only once **for all epochs**. If you are, for example, trying to optimise multiple SMA timeperiod values, the hyperoptable timeperiod parameter should be placed in `populate_entry_trend()` which is calculated every epoch. See [Optimizing an indicator parameter](https://www.Xplitrade.io/en/stable/hyperopt/#optimizing-an-indicator-parameter).
