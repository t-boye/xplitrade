...
The xplitrade documentation describes various ways to install xplitrade
...
!!! Note "Updating"
    Keeping xplitrade updated is important to [ensure ongoing compatibility](updating.md#why-update) with exchange API's.
...
!!! Note "Windows users"
    ...
    If you really want to install xplitrade natively on Windows, best use the [`./setup.ps1` installation script](#use-setupps1-windows).
...
The easiest way to install and run Xplitrade is to clone the bot Github repository and then run the `./setup.sh` (`./setup.ps1` for Windows) script, if it's available for your platform.
...
!!! Note
    Either [uv](https://docs.astral.sh/uv/), or Python3.11 or higher and the corresponding `pip` are assumed to be available. The install-script will warn you and stop if that's not the case. `git` is also needed to clone the Xplitrade repository.  
...
!!! Note "ARM64 systems"
    If you are running an ARM64 system (like a MacOS M1 or an Oracle VM), please use [docker](docker_quickstart.md) to run xplitrade.
...
=== "RaspberryPi/Raspbian"
    ...
    This image comes with python3.11 preinstalled, making it easy to get xplitrade up and running.
...
## Xplitrade repository

Xplitrade is an open source crypto-currency trading bot, whose code is hosted on `github.com`
...
??? Note "Install from pypi"
    An alternative way to install Xplitrade is from [pypi](https://pypi.org/project/xplitrade/). The downside is that this method requires ta-lib to be correctly installed beforehand, and is therefore currently not the recommended way to install Xplitrade.
...
First of the ways to install Xplitrade, is to use provided the Linux/MacOS `./setup.sh` script, which install all dependencies and help you configure the bot.

Make sure you fulfill the [Requirements](#requirements) and have downloaded the [Xplitrade repository](#xplitrade-repository).

### Use /setup.sh -install (Linux/MacOS)

If you are on Debian, Ubuntu or MacOS, xplitrade provides the script to install xplitrade.

```bash
# --install, Install xplitrade from scratch
./setup.sh -i
```
...
You will run xplitrade in separated `virtual environment`

```bash
# create virtualenv in directory /xplitrade/.venv
python3 -m venv .venv
...
Xplitrade can also be installed with Miniconda or Anaconda. We recommend using Miniconda as it's installation footprint is smaller. Conda will automatically prepare and manage the extensive library-dependencies of the Xplitrade program.
...
Download and install xplitrade.
...
#### Xplitrade install: Conda Environment

```bash
conda create --name xplitrade python=3.12
```
...
#### Enter/exit xplitrade environment
...
# activate xplitrade environment
conda activate xplitrade
...
You've made it this far, so you have successfully installed xplitrade.

### Initialize the configuration

```bash
# Step 1 - Initialize user folder
xplitrade create-userdir --userdir user_data

# Step 2 - Create a new configuration file
xplitrade new-config --config user_data/config.json
```
...
```bash
xplitrade trade --config user_data/config.json --strategy SampleStrategy
```
...
```bash
# if:
bash: xplitrade: command not found
