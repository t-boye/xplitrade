...
!!! Note
    If you've used `setup.sh`, don't forget to activate your virtual environment (`source .venv/bin/activate`) before running xplitrade commands.
...
```bash
xplitrade trade -c path/far/far/away/config.json
```
...
```bash
xplitrade trade -c ./config.json
```

and specify both configuration files when running in the normal Live Trade Mode:

```bash
xplitrade trade -c ./config.json -c path/to/secrets/keys.config.json
```
...
Xplitrade allows the creation of a user-data directory using `xplitrade create-userdir --userdir someDirectory`.
...
```bash
xplitrade trade --strategy AwesomeStrategy --strategy-path /some/directory
```
...
```bash
xplitrade trade -c config.json --db-url sqlite:///tradesv3.dry_run.sqlite
```
