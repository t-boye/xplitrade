...
from xplitrade_client.ft_client import add_arguments, main_exec
...
def test_ft_client(mocker, capsys, caplog):
    ...
    mock = mocker.patch("xplitrade_client.ft_client.FtRestClient._call")
...
def test_ft_client_argparsing(mocker, params, expected_args, expected_kwargs, caplog):
    ...
    mocker.patch("xplitrade_client.ft_client.load_config", return_value={}, autospec=True)
    mm = mocker.patch(
        f"xplitrade_client.ft_client.FtRestClient.{mocked_method}", return_value={}, autospec=True
    )
