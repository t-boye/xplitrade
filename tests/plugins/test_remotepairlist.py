...
from xplitrade.plugins.pairlist.RemotePairList import RemotePairList
from xplitrade.plugins.pairlistmanager import PairListManager
from tests.conftest import EXMS, get_patched_exchange, get_patched_xplitradebot, log_has
...
def test_gen_pairlist_with_local_file(mocker, rpl_config):
    ...
    mocker.patch("xplitrade.plugins.pairlist.RemotePairList.open", return_value=mock_file)

    mock_file_path = mocker.patch("xplitrade.plugins.pairlist.RemotePairList.Path")
    mock_file_path.exists.return_value = True

    jsonparse = json.loads(mock_file.read.return_value)
    mocker.patch("xplitrade.plugins.pairlist.RemotePairList.rapidjson.load", return_value=jsonparse)
...
def test_fetch_pairlist_mock_response_html(mocker, rpl_config):
    ...
    mocker.patch(
        "xplitrade.plugins.pairlist.RemotePairList.requests.get", return_value=mock_response
    )
    remote_pairlist = RemotePairList(
        exchange, pairlistmanager, rpl_config, rpl_config["pairlists"][0], 0
    )
...
def test_fetch_pairlist_timeout_keep_last_pairlist(mocker, rpl_config, caplog):
    ...
    mocker.patch(
        "xplitrade.plugins.pairlist.RemotePairList.requests.get",
...
def test_remote_pairlist_init_no_pairlist_url(mocker, rpl_config):
    ...
    with pytest.raises(
        ...
        get_patched_xplitradebot(mocker, rpl_config)
