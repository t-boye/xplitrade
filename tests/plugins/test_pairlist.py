...
from xplitrade.constants import AVAILABLE_PAIRLISTS
from xplitrade.data.dataprovider import DataProvider
from xplitrade.enums import CandleType, RunMode
from xplitrade.exceptions import OperationalException
from xplitrade.persistence import LocalTrade, Trade
from xplitrade.plugins.pairlist.pairlist_helpers import dynamic_expand_pairlist, expand_pairlist
from xplitrade.plugins.pairlistmanager import PairListManager
from xplitrade.resolvers import PairListResolver
from xplitrade.util import dt_now, dt_utc
...
def test_log_cached(mocker, static_pl_conf, markets, tickers):
    ...
    xplitrade = get_patched_xplitradebot(mocker, static_pl_conf)
    logmock = MagicMock()
    # Assign starting whitelist
    pl = xplitrade.pairlists._pairlist_handlers[0]
...
def test_load_pairlist_noexist(mocker, markets, default_conf):
    ...
    plm = PairListManager(xplitrade.exchange, default_conf, MagicMock())
    with pytest.raises(
        ...
        PairListResolver.load_pairlist(
            "NonexistingPairList", xplitrade.exchange, plm, default_conf, {}, 1
        )
...
def test_refresh_market_pair_not_in_whitelist(mocker, markets, static_pl_conf):
    ...
    mocker.patch(f"{EXMS}.markets", PropertyMock(return_value=markets))
    xplitrade.pairlists.refresh_pairlist()
    # List ordered by BaseVolume
    whitelist = ["ETH/BTC", "TKN/BTC"]
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(xplitrade.pairlists.whitelist)
    # Ensure config dict hasn't been changed
    assert (
        static_pl_conf["exchange"]["pair_whitelist"]
        == xplitrade.config["exchange"]["pair_whitelist"]
    )
...
def test_refresh_static_pairlist(mocker, markets, static_pl_conf):
    ...
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(xplitrade.pairlists.whitelist)
    assert static_pl_conf["exchange"]["pair_blacklist"] == xplitrade.pairlists.blacklist
...
def test_refresh_static_pairlist_noexist(mocker, markets, static_pl_conf, pairs, expected, caplog):
    ...
    # Ensure all except those in whitelist are removed
    assert set(expected) == set(xplitrade.pairlists.whitelist)
    assert static_pl_conf["exchange"]["pair_blacklist"] == xplitrade.pairlists.blacklist
    if not expected:
...
def test_remove_logs_for_pairs_already_in_blacklist(mocker, markets, static_pl_conf, caplog):
    ...
    for _ in range(3):
        new_whitelist = xplitrade.pairlists.verify_blacklist(
...
def test_refresh_pairlist_dynamic(mocker, shitcoinmarkets, tickers, whitelist_conf):
    ...
    xplitrade = get_patched_xplitradebot(mocker, whitelist_conf)
    # Remock markets with shitcoinmarkets since get_patched_xplitradebot uses the markets fixture
    mocker.patch.multiple(
        EXMS,
    ...
    xplitrade.pairlists.refresh_pairlist()
    assert whitelist == xplitrade.pairlists.whitelist
    ...
    with pytest.raises(
        ...
        PairListManager(xplitrade.exchange, whitelist_conf, MagicMock())
...
def test_refresh_pairlist_dynamic_2(mocker, shitcoinmarkets, tickers, whitelist_conf_2):
    ...
    mocker.patch.multiple(
        "xplitrade.plugins.pairlistmanager.PairListManager",
        _get_cached_tickers=MagicMock(return_value=tickers_dict),
    )
    xplitrade = get_patched_xplitradebot(mocker, whitelist_conf_2)
    # Remock markets with shitcoinmarkets since get_patched_xplitradebot uses the markets fixture
    mocker.patch.multiple(
        EXMS,
        markets=PropertyMock(return_value=shitcoinmarkets),
...
def test_VolumePairList_whitelist_gen(
    ...
    mocker.patch.multiple(
        "xplitrade.persistence.Trade", get_overall_performance=MagicMock(return_value=[])
    )
    ...
    if whitelist_result == "filter_at_the_beginning":
        ...
        xplitrade.pairlists.refresh_pairlist()
        whitelist = xplitrade.pairlists.whitelist
...
def test_gen_pair_whitelist_not_supported(mocker, default_conf, tickers) -> None:
    ...
    with pytest.raises(
        ...
        get_patched_xplitradebot(mocker, default_conf)
...
def test_pairlist_class(mocker, whitelist_conf, markets, pairlist):
    ...
    assert xplitrade.pairlists.name_list == [pairlist]
    assert pairlist in str(xplitrade.pairlists.short_desc())
    assert isinstance(xplitrade.pairlists.whitelist, list)
    assert isinstance(xplitrade.pairlists.blacklist, list)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["ETH/BTC", "TKN/BTC", "XLTCUSDT"], "is not tradable with Xplitrade"),
    ],
)
def test__whitelist_for_active_markets(
    ...
    # Assign starting whitelist
    pairlist_handler = xplitrade.pairlists._pairlist_handlers[0]
...
def test_volumepairlist_no_range(mocker, whitelist_conf, lookback_conf, caplog):
    ...
    with pytest.raises(
        ...
        get_patched_xplitradebot(mocker, whitelist_conf)
...
def test_volumepairlist_caching(mocker, markets, whitelist_conf, tickers):
    ...
    xplitrade = get_patched_xplitradebot(mocker, whitelist_conf)
    assert len(xplitrade.pairlists._pairlist_handlers[0]._pair_cache) == 0
    assert tickers.call_count == 0
    xplitrade.pairlists.refresh_pairlist()
    assert tickers.call_count == 1

    assert len(xplitrade.pairlists._pairlist_handlers[0]._pair_cache) == 1
    xplitrade.pairlists.refresh_pairlist()
    assert tickers.call_count == 1
...
def test_agefilter_caching(mocker, markets, whitelist_conf_agefilter, tickers, ohlcv_history):
    with time_machine.travel("2021-09-01 05:00:00 +00:00") as t:
        ...
        xplitrade = get_patched_xplitradebot(mocker, whitelist_conf_agefilter)
        assert xplitrade.exchange.refresh_latest_ohlcv.call_count == 0
        xplitrade.pairlists.refresh_pairlist()
        assert len(xplitrade.pairlists.whitelist) == 3
        assert xplitrade.exchange.refresh_latest_ohlcv.call_count > 0

        xplitrade.pairlists.refresh_pairlist()
        assert len(xplitrade.pairlists.whitelist) == 3
        # Call to XRP/BTC cached
        assert xplitrade.exchange.refresh_latest_ohlcv.call_count == 2
        ...
        xplitrade.pairlists.refresh_pairlist()
        assert len(xplitrade.pairlists.whitelist) == 3
        assert xplitrade.exchange.refresh_latest_ohlcv.call_count == 1
        ...
        mocker.patch(f"{EXMS}.refresh_latest_ohlcv", return_value=ohlcv_data)
        xplitrade.pairlists.refresh_pairlist()
        assert len(xplitrade.pairlists.whitelist) == 4
...
def test_rangestabilityfilter_caching(
    ...
    assert xplitrade.exchange.refresh_latest_ohlcv.call_count == 0
    xplitrade.pairlists.refresh_pairlist()
    assert len(xplitrade.pairlists.whitelist) == expected_length
    assert xplitrade.exchange.refresh_latest_ohlcv.call_count > 0

    previous_call_count = xplitrade.exchange.refresh_latest_ohlcv.call_count
    xplitrade.pairlists.refresh_pairlist()
    assert len(xplitrade.pairlists.whitelist) == expected_length
    # Should not have increased since first call.
    assert xplitrade.exchange.refresh_latest_ohlcv.call_count == previous_call_count
...
def test_range_volatility_filter_lookback_timeframe(
    ...
    xplitrade = get_patched_xplitradebot(mocker, default_conf)
    xplitrade.pairlists.refresh_pairlist()
    assert len(xplitrade.pairlists.whitelist) == 5
...
def test_pricefilter_desc(
    ...
    if desc_expected is not None:
        xplitrade = get_patched_xplitradebot(mocker, whitelist_conf)
        short_desc = str(xplitrade.pairlists.short_desc())
...
def test_performance_filter(
    ...
    xplitrade = get_patched_xplitradebot(mocker, allowlist_conf)
    ...
    mocker.patch.multiple(
        "xplitrade.persistence.Trade",
        get_overall_performance=MagicMock(return_value=overall_performance),
    )
    xplitrade.pairlists.refresh_pairlist()
    allowlist = xplitrade.pairlists.whitelist
...
def test_MarketCapPairList_filter(
    ...
    mocker.patch(
        "xplitrade.plugins.pairlist.MarketCapPairList.FtCoinGeckoApi.get_coins_categories_list",
        return_value=[
            {"category_id": "layer-1"},
    ...
    gcm_mock = mocker.patch(
        "xplitrade.plugins.pairlist.MarketCapPairList.FtCoinGeckoApi.get_coins_markets",
...
def test_CrossMarketPairList_breaks_prefix_loop_on_match(mocker, default_conf_usdt, markets):
    ...
    mocker.patch(
        "xplitrade.plugins.pairlist.CrossMarketPairList.PairPrefixes",
