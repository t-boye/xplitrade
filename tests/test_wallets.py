...
from xplitrade.constants import UNLIMITED_STAKE_AMOUNT
from xplitrade.exceptions import DependencyException
from xplitrade.persistence import Trade, WalletHistory
...
def test_sync_wallet_at_boot(mocker, default_conf):
    ...
    assert len(xplitrade.wallets._wallets) == 3
    assert xplitrade.wallets._wallets["BNT"].free == 1.0
    assert xplitrade.wallets._wallets["BNT"].used == 2.0
    assert xplitrade.wallets._wallets["BNT"].total == 3.0
    assert xplitrade.wallets._wallets["GAS"].free == 0.260739
    assert xplitrade.wallets._wallets["GAS"].used == 0.0
    assert xplitrade.wallets._wallets["GAS"].total == 0.260739
    assert xplitrade.wallets.get_free("BNT") == 1.0
    assert "USDT" in xplitrade.wallets._wallets
    assert xplitrade.wallets._last_wallet_refresh is not None
    mocker.patch.multiple(
        EXMS,
        get_balances=MagicMock(
            return_value={
    ...
    assert len(xplitrade.wallets._wallets) == 2
    assert xplitrade.wallets._wallets["BNT"].free == 1.2
    assert xplitrade.wallets._wallets["BNT"].used == 1.9
    assert xplitrade.wallets._wallets["BNT"].total == 3.5
    assert xplitrade.wallets._wallets["GAS"].free == 0.270739
    assert xplitrade.wallets._wallets["GAS"].used == 0.1
    assert xplitrade.wallets._wallets["GAS"].total == 0.260439
    assert xplitrade.wallets.get_free("GAS") == 0.270739
    assert xplitrade.wallets.get_used("GAS") == 0.1
    assert xplitrade.wallets.get_total("GAS") == 0.260439
    assert xplitrade.wallets.get_owned("GAS/USDT", "GAS") == 0.260439
    update_mock = mocker.patch("xplitrade.wallets.Wallets._update_live")
    xplitrade.wallets.update(False)
    assert update_mock.call_count == 0
    xplitrade.wallets.update()
    assert update_mock.call_count == 1

    assert xplitrade.wallets.get_free("NOCURRENCY") == 0
    assert xplitrade.wallets.get_used("NOCURRENCY") == 0
    assert xplitrade.wallets.get_total("NOCURRENCY") == 0
    assert xplitrade.wallets.get_owned("NOCURRENCY/USDT", "NOCURRENCY") == 0
...
def test_sync_wallet_missing_data(mocker, default_conf):
    ...
    assert xplitrade.wallets._wallets["GAS"].used == 0.0
    assert xplitrade.wallets._wallets["GAS"].total == 0.260739
    assert xplitrade.wallets.get_free("GAS") == 0.260739
...
def test_get_trade_stake_amount_no_stake_amount(default_conf, mocker) -> None:
    ...
    with pytest.raises(DependencyException, match=r".*stake amount.*"):
        xplitrade.wallets.get_trade_stake_amount("ETH/BTC", 1)
...
def test_get_trade_stake_amount_unlimited_amount(
    ...
    xplitrade = get_patched_xplitradebot(mocker, conf)

    # no open trades, order amount should be 'balance / max_open_trades'
    result = xplitrade.wallets.get_trade_stake_amount("ETH/USDT", 2)
    assert result == result1

    # create one trade, order amount should be 'balance / (max_open_trades - num_open_trades)'
    xplitrade.execute_entry("ETH/USDT", result)

    result = xplitrade.wallets.get_trade_stake_amount("LTC/USDT", 2)
    assert result == result1

    # create 2 trades, order amount should be None
    xplitrade.execute_entry("LTC/BTC", result)

    result = xplitrade.wallets.get_trade_stake_amount("XRP/USDT", 2)
    assert result == 0

    xplitrade.config["dry_run_wallet"] = 200
    xplitrade.wallets._start_cap["BTC"] = 200
    result = xplitrade.wallets.get_trade_stake_amount("XRP/USDT", 3)
    assert round(result, 4) == round(result2, 4)

    # set max_open_trades = None, so do not trade
    result = xplitrade.wallets.get_trade_stake_amount("NEO/USDT", 0)
    assert result == 0
...
def test_validate_stake_amount(
    ...
    mocker.patch(
        "xplitrade.wallets.Wallets.get_available_stake_amount", return_value=stake_available
    )
    res = xplitrade.wallets.validate_stake_amount(
...
def test_get_starting_balance(
    ...
    mocker.patch(
        "xplitrade.persistence.models.Trade.get_total_closed_profit", return_value=closed_profit
    )
    mocker.patch(
        "xplitrade.persistence.models.Trade.total_open_trades_stakes", return_value=open_stakes
    )
    mocker.patch("xplitrade.wallets.Wallets.get_free", return_value=free)

    xplitrade = get_patched_xplitradebot(mocker, default_conf)

    assert xplitrade.wallets.get_starting_balance() == expected * (1 if available_capital else 0.99)
...
def test_sync_wallet_futures_live(mocker, default_conf):
    ...
    assert len(xplitrade.wallets._wallets) == 1
    assert len(xplitrade.wallets._positions) == 2

    assert "USDT" in xplitrade.wallets._wallets
    assert "ETH/USDT:USDT" in xplitrade.wallets._positions
    assert xplitrade.wallets._last_wallet_refresh is not None
    assert xplitrade.wallets.get_owned("ETH/USDT:USDT", "ETH") == 1000
    assert xplitrade.wallets.get_owned("SOL/USDT:USDT", "SOL") == 0

    # Remove ETH/USDT:USDT position
    del mock_result[0]
    xplitrade.wallets.update()
    assert len(xplitrade.wallets._positions) == 1
    assert "ETH/USDT:USDT" not in xplitrade.wallets._positions
...
def test_sync_wallet_futures_live_no_positions_unchanged(mocker, default_conf_usdt):
    ...
    xplitrade = get_patched_xplitradebot(mocker, default_conf_usdt)
    assert xplitrade.wallets.get_total("USDT") == 1000
...
def test_sync_wallet_dry(mocker, default_conf_usdt, fee):
    ...
    xplitrade = get_patched_xplitradebot(mocker, default_conf_usdt)
    assert len(xplitrade.wallets._wallets) == 1
    assert len(xplitrade.wallets._positions) == 0
    assert xplitrade.wallets.get_total("USDT") == 1000
    ...
    assert len(xplitrade.wallets._wallets) == 5
    assert len(xplitrade.wallets._positions) == 0
    bal = xplitrade.wallets.get_all_balances()
    ...
    assert (
        xplitrade.wallets.get_starting_balance()
        == default_conf_usdt["dry_run_wallet"] * default_conf_usdt["tradable_balance_ratio"]
    )
    total = xplitrade.wallets.get_total("LTC")
    free = xplitrade.wallets.get_free("LTC")
    used = xplitrade.wallets.get_used("LTC")
    assert used != 0
...
def test_sync_wallet_futures_dry(mocker, default_conf, fee):
    ...
    assert len(xplitrade.wallets._wallets) == 1
    assert len(xplitrade.wallets._positions) == 4
    positions = xplitrade.wallets.get_all_positions()
    ...
    total = xplitrade.wallets.get_total("BTC")
    free = xplitrade.wallets.get_free("BTC")
    used = xplitrade.wallets.get_used("BTC")
    assert free + used == total


def test_check_exit_amount(mocker, default_conf, fee):
    xplitrade = get_patched_xplitradebot(mocker, default_conf)
    update_mock = mocker.patch("xplitrade.wallets.Wallets.update")
    total_mock = mocker.patch("xplitrade.wallets.Wallets.get_total", return_value=50.0)

    create_mock_trades(fee, is_short=None)
    trade = Trade.session.scalars(select(Trade)).first()
    assert trade.amount == 50.0

    assert xplitrade.wallets.check_exit_amount(trade) is True
    assert update_mock.call_count == 0
    ...
    total_mock = mocker.patch("xplitrade.wallets.Wallets.get_total", return_value=40)
    assert xplitrade.wallets.check_exit_amount(trade) is False
    assert update_mock.call_count == 1
...
def test_check_exit_amount_futures(mocker, default_conf, fee):
    ...
    default_conf["margin_mode"] = "isolated"
    xplitrade = get_patched_xplitradebot(mocker, default_conf)
    total_mock = mocker.patch("xplitrade.wallets.Wallets.get_total", return_value=50)
...
def test_dry_run_wallet_initialization(mocker, default_conf_usdt, config, wallets):
    ...
    for currency, expected_wallet in wallets.items():
        wallet = xplitrade.wallets._wallets[currency]
    ...
    assert len(xplitrade.wallets._wallets) == len(wallets)
    ...
    assert xplitrade.wallets.get_collateral() == xplitrade.wallets.get_free(stake_currency)
    xplitrade.execute_entry("NEO/USDT", 100.0)

    # Update wallets and verify NEO is now included
    xplitrade.wallets.update()
    if default_conf_usdt["trading_mode"] != "futures":
        assert "NEO" in xplitrade.wallets._wallets

        assert xplitrade.wallets._wallets["NEO"].total == 45.04504504  # 100 USDT / 0.22
        assert xplitrade.wallets._wallets["NEO"].used == 0.0
        assert xplitrade.wallets._wallets["NEO"].free == 45.04504504
        assert xplitrade.wallets.get_collateral() == xplitrade.wallets.get_free(stake_currency)
        # Verify USDT wallet was reduced by trade amount
        assert (
            pytest.approx(xplitrade.wallets._wallets[stake_currency].total)
            == wallets[stake_currency]["total"] - 100.0
        )
        assert len(xplitrade.wallets._wallets) == len(wallets) + 1  # Original wallets + NEO
    else:
        # Futures mode
        assert "NEO" not in xplitrade.wallets._wallets
        assert xplitrade.wallets._positions["NEO/USDT"].position == 45.04504504
        assert pytest.approx(xplitrade.wallets._positions["NEO/USDT"].collateral) == 100

        # Verify USDT wallet's free was reduced by trade amount
        assert (
            pytest.approx(xplitrade.wallets.get_collateral())
            == xplitrade.wallets.get_free(stake_currency) + 100
        )
        assert (
            pytest.approx(xplitrade.wallets._wallets[stake_currency].free)
...
def test_record_wallet_state_stores_wallet_history(mocker, default_conf_usdt):
    ...
    mocker.patch(
        "xplitrade.persistence.trade_model.Trade.get_open_trades",
        return_value=[
            MagicMock(pair="ETH/USDT:USDT", safe_base_currency="ETH"),
        ],
    )

    xplitrade.wallets.record_wallet_state()
