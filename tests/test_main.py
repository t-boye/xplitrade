...
from xplitrade.enums import State
...
from xplitrade.main import main
...
def test_parse_args_version(capsys) -> None:
    ...
    assert re.search(r"Xplitrade Version:\s+xplitrade\s.*", captured.out, re.MULTILINE)
...
def test_parse_args_backtesting(mocker) -> None:
    ...
    backtesting_mock = mocker.patch("xplitrade.commands.start_backtesting")
...
def test_main_start_hyperopt(mocker) -> None:
    mocker.patch.object(Path, "is_file", MagicMock(side_effect=[False, True]))
    hyperopt_mock = mocker.patch("xplitrade.commands.start_hyperopt", MagicMock())
...
def test_main_fatal_exception(mocker, default_conf, caplog) -> None:
    ...
    mocker.patch("xplitrade.xplitradebot.XplitradeBot.cleanup", MagicMock())
    mocker.patch("xplitrade.worker.Worker._worker", MagicMock(side_effect=Exception))
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch("xplitrade.xplitradebot.RPCManager", MagicMock())
    mocker.patch("xplitrade.xplitradebot.init_db", MagicMock())
...
def test_main_keyboard_interrupt(mocker, default_conf, caplog) -> None:
    patch_exchange(mocker)
    mocker.patch("xplitrade.xplitradebot.XplitradeBot.cleanup", MagicMock())
    mocker.patch("xplitrade.worker.Worker._worker", MagicMock(side_effect=KeyboardInterrupt))
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch("xplitrade.xplitradebot.RPCManager", MagicMock())
    mocker.patch("xplitrade.wallets.Wallets.update", MagicMock())
    mocker.patch("xplitrade.xplitradebot.init_db", MagicMock())
...
def test_main_operational_exception(mocker, default_conf, caplog) -> None:
    ...
    mocker.patch("xplitrade.xplitradebot.XplitradeBot.cleanup", MagicMock())
    mocker.patch(
        "xplitrade.worker.Worker._worker", MagicMock(side_effect=XplitradeException("Oh snap!"))
    )
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch("xplitrade.wallets.Wallets.update", MagicMock())
    mocker.patch("xplitrade.xplitradebot.RPCManager", MagicMock())
...
def test_main_operational_exception1(mocker, default_conf, caplog) -> None:
    patch_exchange(mocker)
    mocker.patch(
        "xplitrade.exchange.list_available_exchanges",
...
def test_main_reload_config(mocker, default_conf, caplog) -> None:
    ...
    mocker.patch("xplitrade.worker.Worker._worker", worker_mock)
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch("xplitrade.wallets.Wallets.update", MagicMock())
    reconfigure_mock = mocker.patch("xplitrade.worker.Worker._reconfigure", MagicMock())
    ...
    assert isinstance(worker.xplitrade, XplitradeBot)
...
def test_reconfigure(mocker, default_conf) -> None:
    ...
    mocker.patch("xplitrade.xplitradebot.XplitradeBot.cleanup", MagicMock())
    mocker.patch(
        "xplitrade.worker.Worker._worker", MagicMock(side_effect=OperationalException("Oh snap!"))
    )
    mocker.patch("xplitrade.wallets.Wallets.update", MagicMock())
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch("xplitrade.xplitradebot.RPCManager", MagicMock())
