...
from xplitrade.loggers import (
    FTBufferingHandler,
    FtRichHandler,
    setup_logging,
    setup_logging_pre,
)
from xplitrade.loggers.set_log_levels import (
...
def test_reduce_verbosity():
    setup_logging_pre()
    reduce_verbosity_for_bias_tester()
    prior_level = logging.getLogger("xplitrade").getEffectiveLevel()

    assert logging.getLogger("xplitrade.resolvers").getEffectiveLevel() == logging.WARNING
    assert logging.getLogger("xplitrade.strategy.hyper").getEffectiveLevel() == logging.WARNING
    # base level wasn't changed
    assert logging.getLogger("xplitrade").getEffectiveLevel() == prior_level

    restore_verbosity_for_bias_tester()

    assert logging.getLogger("xplitrade.resolvers").getEffectiveLevel() == prior_level
    assert logging.getLogger("xplitrade.strategy.hyper").getEffectiveLevel() == prior_level
    assert logging.getLogger("xplitrade").getEffectiveLevel() == prior_level
    # base level wasn't changed
