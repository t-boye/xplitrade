...
from xplitrade.commands.build_config_commands import start_new_config
from xplitrade.configuration.deploy_config import (
    ask_user_config,
    ask_user_overwrite,
    validate_is_float,
    validate_is_int,
...
def test_start_new_config(mocker, caplog, exchange):
    ...
    mocker.patch("xplitrade.configuration.deploy_config.ask_user_overwrite", return_value=True)
    ...
    mocker.patch(
        "xplitrade.configuration.deploy_config.ask_user_config", return_value=sample_selections
    )
...
def test_start_new_config_exists(mocker, caplog):
    mocker.patch.object(Path, "exists", MagicMock(return_value=True))
    mocker.patch("xplitrade.configuration.deploy_config.ask_user_overwrite", return_value=False)
    args = ["new-config", "--config", "coolconfig.json"]
...
def test_ask_user_overwrite(mocker):
    ...
    prompt_mock = mocker.patch(
        "xplitrade.configuration.deploy_config.prompt", return_value={"overwrite": False}
    )
    ...
    prompt_mock = mocker.patch(
        "xplitrade.configuration.deploy_config.prompt", return_value={"overwrite": True}
    )
...
def test_ask_user_config(mocker):
    ...
    prompt_mock = mocker.patch("xplitrade.configuration.deploy_config.prompt", return_value={})
