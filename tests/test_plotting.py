...
from xplitrade.commands import start_plot_dataframe, start_plot_profit
from xplitrade.configuration import TimeRange
from xplitrade.data import history
from xplitrade.data.btanalysis import load_backtest_data
from xplitrade.data.metrics import create_cum_profit
from xplitrade.exceptions import OperationalException
from xplitrade.plot.plotting import (
    add_areas,
    add_indicators,
    add_profit,
    create_plotconfig,
...
def test_generate_candlestick_graph_no_signals_no_trades(default_conf, mocker, testdatadir, caplog):
    row_mock = mocker.patch(
        "xplitrade.plot.plotting.add_indicators", MagicMock(side_effect=fig_generating_mock)
    )
    trades_mock = mocker.patch(
        "xplitrade.plot.plotting.plot_trades", MagicMock(side_effect=fig_generating_mock)
    )
...
def test_generate_Plot_filename():
    ...
    assert fn == "xplitrade-plot-UNITTEST_BTC-5m.html"


def test_generate_plot_file(mocker, caplog, user_dir):
    fig = generate_empty_figure()
    plot_mock = mocker.patch("xplitrade.plot.plotting.plot", MagicMock())
    store_plot_file(
...
def test_generate_profit_graph(testdatadir):
    ...
    assert fig.layout.title.text == "Xplitrade Profit plot"
...
def test_start_plot_dataframe(mocker):
    aup = mocker.patch("xplitrade.plot.plotting.load_and_plot_trades", MagicMock())
    args = [
        "plot-dataframe",
        "--config",
...
def test_start_plot_profit(mocker):
    aup = mocker.patch("xplitrade.plot.plotting.plot_profit", MagicMock())
    args = [
        "plot-profit",
        "--config",
        "tests/testdata/testconfigs/main_test_config.json",
...
def test_plot_profit(default_conf, mocker, testdatadir):
    ...
    store_mock = MagicMock()
    mocker.patch.multiple(
        "xplitrade.plot.plotting", generate_profit_graph=profit_mock, store_plot_file=store_mock
    )
    with pytest.raises(
