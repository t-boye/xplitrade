...
from xplitrade.constants import CANCEL_REASON
...
from xplitrade.exceptions import OperationalException
from xplitrade.xplitradebot import XplitradeBot
from xplitrade.loggers import setup_logging
from xplitrade.persistence import PairLocks, Trade
from xplitrade.persistence.models import Order
from xplitrade.rpc import RPC
from xplitrade.rpc.rpc import RPCException
from xplitrade.rpc.telegram import Telegram, authorized_only
from xplitrade.util.datetime_helpers import dt_now
...
def mock_exchange_loop(mocker):
    mocker.patch("xplitrade.exchange.exchange.Exchange._init_async_loop")
...
def get_telegram_testobject(mocker, default_conf, mock=True, ftbot=None, mock_fiat=True):
    ...
    if mock:
        mocker.patch.multiple(
            "xplitrade.rpc.telegram.Telegram",
            _init=MagicMock(),
            _send_msg=msg_mock,
            _start_thread=MagicMock(),
        )
    if not ftbot:
        ftbot = get_patched_xplitradebot(mocker, default_conf)
    rpc = RPC(ftbot)
...
def test_telegram__init__(default_conf, mocker) -> None:
    mocker.patch("xplitrade.rpc.telegram.Telegram._init", MagicMock())
...
def test_telegram_init(default_conf, mocker, caplog) -> None:
    app_mock = MagicMock()
    mocker.patch("xplitrade.rpc.telegram.Telegram._start_thread", MagicMock())
    mocker.patch("xplitrade.rpc.telegram.Telegram._init_telegram_app", return_value=app_mock)
    mocker.patch("xplitrade.rpc.telegram.Telegram._startup_telegram", AsyncMock())
...
async def test_telegram_startup(default_conf, mocker, caplog) -> None:
    ...
    sleep_mock = mocker.patch("xplitrade.rpc.telegram.asyncio.sleep", AsyncMock())
...
async def test_telegram_cleanup(
    ...
    # mocker.patch('xplitrade.rpc.telegram.Application', app_mock)
...
async def test_telegram_status(default_conf, update, mocker) -> None:
    ...
    mocker.patch("xplitrade.rpc.telegram.Telegram._status_table", status_table)

    mocker.patch.multiple(
        "xplitrade.rpc.rpc.RPC",
...
async def test_order_handle(default_conf, update, ticker, fee, mocker) -> None:
    ...
    telegram, xplitradebot, msg_mock = get_telegram_testobject(mocker, default_conf)

    patch_get_signal(xplitradebot)

    xplitradebot.state = State.RUNNING
    msg_mock.reset_mock()

    # Create some test data
    xplitradebot.enter_positions(3)

    mocker.patch("xplitrade.rpc.telegram.MAX_MESSAGE_LENGTH", 500)
    ...
    msg_mock.reset_mock()
    mocker.patch("xplitrade.rpc.telegram.MAX_MESSAGE_LENGTH", 50)
    context = MagicMock()
    context.args = ["2"]
    await telegram._order(update=update, context=context)
...
async def test_status_handle(default_conf, update, ticker, fee, mocker) -> None:
    ...
    xplitradebot.state = State.STOPPED
...
async def test_status_table_handle(default_conf, update, ticker, fee, mocker) -> None:
    ...
    # Create some test data
    xplitradebot.enter_positions(1)
...
async def test_weekly_handle(default_conf_usdt, update, ticker, fee, mocker, time_machine) -> None:
    ...
    telegram, xplitradebot, msg_mock = get_telegram_testobject(mocker, default_conf_usdt)
...
async def test_telegram_profit_handle(
    ...
    mocker.patch("xplitrade.wallets.Wallets.get_starting_balance", return_value=1000)
    assert (
...
async def test_balance_handle_empty_response(default_conf, update, mocker) -> None:
    ...
    xplitradebot.config["dry_run"] = False
    await telegram._balance(update=update, context=MagicMock())
    result = msg_mock.call_args_list[0][0][0]
    assert msg_mock.call_count == 1
...
async def test_balance_handle_too_large_response(default_conf, update, mocker) -> None:
    ...
    mocker.patch(
        "xplitrade.rpc.rpc.RPC._rpc_balance",
        return_value={
            "currencies": balances,
            "total": 100.0,
            "total_bot": 100.0,
...
async def test_start_handle(default_conf, update, mocker) -> None:
    ...
    xplitradebot.state = State.STOPPED
    assert xplitradebot.state == State.STOPPED
    await telegram._start(update=update, context=MagicMock())
    assert xplitradebot.state == State.RUNNING
    assert msg_mock.call_count == 1
...
async def test_pause_handle(default_conf, update, mocker) -> None:
    ...
    assert xplitradebot.state == State.PAUSED
    assert msg_mock.call_count == 1
    assert (
...
async def test_reload_config_handle(default_conf, update, mocker) -> None:
    ...
    assert xplitradebot.state == State.RELOAD_CONFIG
    assert msg_mock.call_count == 1
...
async def test_telegram_forceexit_handle(
    ...
    msg_mock = mocker.patch("xplitrade.rpc.telegram.Telegram.send_msg", MagicMock())
    mocker.patch("xplitrade.rpc.telegram.Telegram._init", MagicMock())
    patch_exchange(mocker)
    ...
    xplitradebot = XplitradeBot(default_conf)
    rpc = RPC(xplitradebot)
    telegram = Telegram(rpc, default_conf)
    patch_get_signal(xplitradebot)
...
async def test_forceexit_all_handle(default_conf, update, ticker, fee, mocker) -> None:
    ...
    # Create some test data
    xplitradebot.enter_positions(4)
    msg_mock.reset_mock()
...
async def test_force_exit_no_pair(default_conf, update, ticker, fee, mocker) -> None:
    ...
    femock = mocker.patch("xplitrade.rpc.rpc.RPC._rpc_force_exit")
    telegram, xplitradebot, msg_mock = get_telegram_testobject(mocker, default_conf)
...
async def test_force_enter_handle(default_conf, update, mocker) -> None:
    ...
    mocker.patch("xplitrade.rpc.rpc.RPC._rpc_force_entry", fbuy_mock)

    telegram, xplitradebot, _ = get_telegram_testobject(mocker, default_conf)
    patch_get_signal(xplitradebot)
...
async def test_force_enter_no_pair(default_conf, update, mocker) -> None:
    fbuy_mock = mocker.patch("xplitrade.rpc.rpc.RPC._rpc_force_entry", return_value=None)
...
async def test_telegram_entry_tag_performance_handle(
    ...
    msg_mock.reset_mock()
    mocker.patch(
        "xplitrade.rpc.rpc.RPC._rpc_enter_tag_performance", side_effect=RPCException("Error")
    )
...
async def test_telegram_exit_reason_performance_handle(
    ...
    msg_mock.reset_mock()
    mocker.patch(
        "xplitrade.rpc.rpc.RPC._rpc_exit_reason_performance", side_effect=RPCException("Error")
    )
...
async def test_telegram_mix_tag_performance_handle(
    ...
    msg_mock.reset_mock()
    mocker.patch(
        "xplitrade.rpc.rpc.RPC._rpc_mix_tag_performance", side_effect=RPCException("Error")
    )
...
async def test_blacklist_static(default_conf, update, mocker) -> None:
    ...
    assert xplitradebot.pairlists.blacklist == ["DOGE/BTC", "HOT/BTC", "ETH/BTC"]
    ...
    assert xplitradebot.pairlists.blacklist == ["DOGE/BTC", "HOT/BTC", "ETH/BTC", "XRP/.*"]
...
async def test_telegram_logs(default_conf, update, mocker) -> None:
    ...
    assert msg_mock.call_count == 1
    assert "xplitrade\\.rpc\\.telegram" in msg_mock.call_args_list[0][0][0]
    ...
    mocker.patch("xplitrade.rpc.telegram.MAX_MESSAGE_LENGTH", 200)
    context = MagicMock()
    context.args = []
    await telegram._logs(update=update, context=context)
...
async def test_version_handle(default_conf, update, mocker) -> None:
    ...
    msg_mock.reset_mock()
    xplitradebot.strategy.version = lambda: "1.1.1"
...
async def test_show_config_handle(default_conf, update, mocker) -> None:
    ...
    msg_mock.reset_mock()
    xplitradebot.config["trailing_stop"] = True
    await telegram._show_config(update=update, context=MagicMock())
    assert msg_mock.call_count == 1
    assert "*Mode:* `{}`".format("Dry-run") in msg_mock.call_args_list[0][0][0]
    assert "*Exchange:* `binance`" in msg_mock.call_args_list[0][0][0]
...
def test_send_msg_enter_notification(
    ...
    mocker.patch(
        "xplitrade.data.dataprovider.DataProvider.get_analyzed_dataframe", return_value=(df, 1)
    )
    ...
    telegram, xplitradebot, msg_mock = get_telegram_testobject(
    ...
    xplitradebot.config["telegram"]["notification_settings"] = {"entry": "off"}
    caplog.clear()
    msg_mock.reset_mock()
    telegram.send_msg(msg)
    ...
    xplitradebot.config["telegram"]["notification_settings"] = {"entry": "silent"}
    caplog.clear()
    msg_mock.reset_mock()
...
async def test__send_msg_keyboard(default_conf, mocker, caplog) -> None:
    ...
    bot = MagicMock()
    bot.send_message = AsyncMock()
    xplitradebot = get_patched_xplitradebot(mocker, default_conf)
    rpc = RPC(xplitradebot)
    ...
    def init_telegram(xplitradebot):
        telegram = Telegram(rpc, default_conf)
        telegram._app = MagicMock()
        telegram._app.bot = bot
        return telegram
    ...
    xplitradebot.config["telegram"]["enabled"] = True
    telegram = init_telegram(xplitradebot)
    await telegram._send_msg("test")
    ...
    xplitradebot.config["telegram"]["enabled"] = True
    xplitradebot.config["telegram"]["keyboard"] = invalid_keys_list
    err_msg = (
        re.escape(
    ...
    xplitradebot.config["telegram"]["enabled"] = True
    xplitradebot.config["telegram"]["keyboard"] = custom_keys_list
    telegram = init_telegram(xplitradebot)
    await telegram._send_msg("test")
    used_keyboard = bot.send_message.call_args[1]["reply_markup"]
