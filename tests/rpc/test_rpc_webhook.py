...
from xplitrade.enums import ExitType, RPCMessageType
from xplitrade.rpc import RPC
from xplitrade.rpc.discord import Discord
from xplitrade.rpc.webhook import Webhook
...
def test_send_msg_webhook(default_conf, mocker):
    ...
    msg_mock = MagicMock()
    mocker.patch("xplitrade.rpc.webhook.Webhook._send_msg", msg_mock)
    webhook = Webhook(RPC(get_patched_xplitradebot(mocker, default_conf)), default_conf)
    # Test buy
    msg_mock = MagicMock()
    mocker.patch("xplitrade.rpc.webhook.Webhook._send_msg", msg_mock)
...
def test_send_msg_discord(default_conf, mocker):
    ...
    msg_mock = MagicMock()
    mocker.patch("xplitrade.rpc.webhook.Webhook._send_msg", msg_mock)
    discord = Discord(RPC(get_patched_xplitradebot(mocker, default_conf)), default_conf)
...
def test_nested_payload_format(default_conf, mocker):
    ...
    post = mocker.patch("xplitrade.rpc.webhook.post")
    webhook.send_msg(msg)
