...
from xplitrade.enums import SignalDirection, State, TradingMode
from xplitrade.exceptions import ExchangeError, InvalidOrderException, TemporaryError
from xplitrade.persistence import Order, Trade
from xplitrade.persistence.key_value_store import set_startup_time
from xplitrade.rpc import RPC, RPCException
...
def test_rpc_trade_status(default_conf, ticker, fee, mocker) -> None:
    ...
    # Fill open order ...
    xplitradebot.manage_open_orders()
    trades = Trade.get_open_trades()
    xplitradebot.exit_positions(trades)
...
def test__rpc_timeunit_profit(
    ...
    xplitradebot = get_patched_xplitradebot(mocker, default_conf_usdt)
...
def test_rpc_delete_trade(mocker, default_conf, fee, markets, caplog, is_short):
    ...
    xplitradebot = get_patched_xplitradebot(mocker, default_conf)
    mocker.patch.multiple(
        xplitradebot.exchange,
        cancel_order=cancel_mock,
        cancel_stoploss_order=stoploss_mock,
    )
    xplitradebot.strategy.order_types["stoploss_on_exchange"] = True
    create_mock_trades(fee, is_short)
    rpc = RPC(xplitradebot)
    ...
    stoploss_mock = mocker.patch.object(
        xplitradebot.exchange, "cancel_stoploss_order", side_effect=InvalidOrderException
    )
    ...
    cancel_mock = mocker.patch.object(
        xplitradebot.exchange, "cancel_order", side_effect=InvalidOrderException
    )
...
def test_rpc_pause(mocker, default_conf) -> None:
    ...
    patch_get_signal(xplitradebot)
    rpc = RPC(xplitradebot)
    xplitradebot.state = State.PAUSED
...
def test_rpc_force_exit(default_conf, ticker, fee, mocker) -> None:
    ...
    xplitradebot.config["max_open_trades"] = 3
...
def test_rpc_force_entry(mocker, default_conf, ticker, fee, limit_buy_order_open) -> None:
    ...
    xplitradebot.config["max_open_trades"] = 5
    ...
    xplitradebot.strategy.position_adjustment_enable = True
    ...
    pair = "XRP/BTC"
    xplitradebot = get_patched_xplitradebot(mocker, default_conf)
    xplitradebot.config["stake_amount"] = 0
    patch_get_signal(xplitradebot)
    rpc = RPC(xplitradebot)
    pair = "TKN/BTC"
