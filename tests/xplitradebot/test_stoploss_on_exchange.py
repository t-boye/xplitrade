...
from xplitrade.enums import ExitCheckTuple, ExitType, RPCMessageType
from xplitrade.exceptions import ExchangeError, InsufficientFundsError, InvalidOrderException
from xplitrade.xplitradebot import XplitradeBot
from xplitrade.persistence import Order, Trade
from xplitrade.persistence.models import PairLock
...
def test_add_stoploss_on_exchange(mocker, default_conf_usdt, limit_order, is_short, fee) -> None:
    ...
    xplitrade.exit_positions(trades)
...
def test_handle_stoploss_on_exchange(
    ...
    assert xplitrade.handle_stoploss_on_exchange(trade) is False
    assert stoploss.call_count == 1
    ...
    mocker.patch.object(xplitrade.exchange, "fetch_stoploss_order", hanging_stoploss_order)
    ...
    mocker.patch.object(xplitrade.exchange, "fetch_stoploss_order", canceled_stoploss_order)
    stoploss.reset_mock()
    ...
    mocker.patch.object(xplitrade.exchange, "fetch_stoploss_order", stoploss_order_hit)
    xplitrade.strategy.order_filled = MagicMock(return_value=None)
    assert xplitrade.handle_stoploss_on_exchange(trade) is True
    ...
    mocker.patch.object(xplitrade.exchange, "create_stoploss", side_effect=ExchangeError())
    trade.is_open = True
    xplitrade.handle_stoploss_on_exchange(trade)
    ...
    stoploss.reset_mock()
    mocker.patch.multiple(xplitrade.exchange, fetch_order=MagicMock(), create_stoploss=stoploss)
    assert xplitrade.handle_stoploss_on_exchange(trade) is False
    assert trade.has_open_sl_orders is False
...
def test_handle_stoploss_on_exchange_emergency(
    ...
    xplitrade.config["trailing_stop"] = True
...
def test_handle_stoploss_on_exchange_partial(
    ...
    mocker.patch.multiple(xplitrade.exchange, fetch_stoploss_order=stoploss_order_hit)
    assert xplitrade.handle_stoploss_on_exchange(trade) is False
...
def test_create_stoploss_order_invalid_order(
    ...
    caplog.clear()
    rpc_mock.reset_mock()
    xplitrade.create_stoploss_order(trade, 200)
    assert trade.has_open_sl_orders is False
...
def test_handle_stoploss_on_exchange_trailing(
    ...
    # setting stoploss
    xplitrade.strategy.stoploss = 0.05 if is_short else -0.05

    # setting stoploss_on_exchange_interval to 60 seconds
    xplitrade.strategy.order_types["stoploss_on_exchange_interval"] = 60
    ...
    stoploss_order_mock.assert_called_once_with(
        amount=30,
        pair="ETH/USDT",
        order_types=xplitrade.strategy.order_types,
...
def test_handle_stoploss_on_exchange_trailing_error(
    ...
    mocker.patch.object(
        xplitrade.exchange, "cancel_stoploss_order", side_effect=InvalidOrderException()
    )
    mocker.patch.object(
        xplitrade.exchange, "fetch_stoploss_order", return_value=stoploss_order_hanging
    )
    time_machine.shift(timedelta(minutes=50))
    xplitrade.handle_trailing_stoploss_on_exchange(trade, stoploss_order_hanging)
    ...
    caplog.clear()
    cancel_mock = mocker.patch.object(xplitrade.exchange, "cancel_stoploss_order")
    mocker.patch.object(xplitrade.exchange, "create_stoploss", side_effect=ExchangeError())
    time_machine.shift(timedelta(minutes=50))
    xplitrade.handle_trailing_stoploss_on_exchange(trade, stoploss_order_hanging)
...
def test_stoploss_on_exchange_price_rounding(
    ...
    xplitrade.handle_trailing_stoploss_on_exchange(open_trade_usdt, {})
...
def test_handle_stoploss_on_exchange_custom_stop(
    ...
    # setting stoploss
    xplitrade.strategy.custom_stoploss = lambda *args, **kwargs: -0.04
    ...
    xplitrade.strategy.order_types["stoploss_on_exchange_interval"] = 0
    cancel_order_mock.assert_not_called()
    stoploss_order_mock.assert_not_called()
...
def test_execute_trade_exit_sloe_cancel_exception(
    ...
    mocker.patch("xplitrade.wallets.Wallets.get_free", MagicMock(return_value=300))
    create_order_mock = MagicMock(
        side_effect=[
            {"id": "12345554"},
            {"id": "12345555"},
...
def test_may_execute_trade_exit_after_stoploss_on_exchange_hit(
    ...
    xplitrade = XplitradeBot(default_conf_usdt)
    xplitrade.strategy.order_types["stoploss_on_exchange"] = True
    patch_get_signal(xplitrade, enter_long=not is_short, enter_short=is_short)
    ...
    mocker.patch.object(xplitrade.exchange, "fetch_stoploss_order", stoploss_executed)
