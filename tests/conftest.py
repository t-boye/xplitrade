...
from xplitrade.data.converter import ohlcv_to_dataframe, trades_list_to_df
from xplitrade.enums import CandleType, MarginMode, SignalDirection, TradingMode
from xplitrade.exchange import Exchange, timeframe_to_minutes, timeframe_to_seconds
from xplitrade.xplitradebot import XplitradeBot
from xplitrade.persistence import LocalTrade, Order, Trade, init_db
from xplitrade.persistence.custom_data import _CustomData
from xplitrade.resolvers import ExchangeResolver
from xplitrade.system import set_mp_start_method
from xplitrade.util import dt_now, dt_ts
...
EXMS = "xplitrade.exchange.exchange.Exchange"
...
def patched_configuration_load_config_file(mocker, config) -> None:
    mocker.patch(
        "xplitrade.configuration.load_config.load_config_file", lambda *args, **kwargs: config
    )
...
def patch_exchange(
    ...
    # Temporary patch ...
    mocker.patch("xplitrade.exchange.bybit.Bybit.cache_leverage_tiers")
    ...
    if mock_supported_modes:
        mocker.patch(
            f"xplitrade.exchange.{exchange}.{exchange.capitalize()}"
...
def patch_wallet(mocker, free=999.9) -> None:
    mocker.patch("xplitrade.wallets.Wallets.get_free", MagicMock(return_value=free))


def patch_whitelist(mocker, conf) -> None:
    mocker.patch(
        "xplitrade.xplitradebot.XplitradeBot._refresh_active_whitelist",
...
def patch_xplitradebot(mocker, config) -> None:
    """
    ...
    :return: None
    """
    mocker.patch("xplitrade.xplitradebot.RPCManager", MagicMock())
    patch_exchange(mocker)
    mocker.patch("xplitrade.xplitradebot.RPCManager._init", MagicMock())
    mocker.patch("xplitrade.xplitradebot.RPCManager.send_msg", MagicMock())
    patch_whitelist(mocker, config)
    mocker.patch("xplitrade.xplitradebot.ExternalMessageConsumer")
    mocker.patch("xplitrade.configuration.config_validation._validate_consumers")


def get_patched_xplitradebot(mocker, config) -> XplitradeBot:
    """
    ...
    :return: XplitradeBot
    """
    patch_xplitradebot(mocker, config)
    return XplitradeBot(config)
...
def patch_get_signal(
    ...
    xplitrade.strategy.get_entry_signal = patched_get_entry_signal
    ...
    # returns (enter, exit)
    xplitrade.strategy.get_exit_signal = patched_get_exit_signal

    xplitrade.exchange.refresh_latest_ohlcv = lambda p: None
...
def patch_gc(mocker) -> None:
    mocker.patch("xplitrade.main.gc_set_threshold")
...
def user_dir(mocker, tmp_path) -> Path:
    ...
    mocker.patch("xplitrade.configuration.configuration.create_userdata_dir", return_value=user_dir)
    return user_dir
...
def patch_coingecko(mocker) -> None:
    ...
    mocker.patch.multiple(
        "xplitrade.rpc.fiat_convert.FtCoinGeckoApi",
