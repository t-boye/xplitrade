...
from xplitrade.constants import DEFAULT_DATAFRAME_COLUMNS
from xplitrade.data.converter import ohlcv_to_dataframe
from xplitrade.enums import CandleType, MarginMode, RunMode, TradingMode
...
from xplitrade.util import dt_now, dt_ts, dt_utc
...
def ccxt_exceptionhandlers(
    ...
    **kwargs,
    ...
    with patch("xplitrade.exchange.common.time.sleep"):
...
async def async_ccxt_exception(
    ...
    with patch("xplitrade.exchange.common.asyncio.sleep"):
        with pytest.raises(DDosProtection):
...
async def test__async_kucoin_get_candle_history(default_conf, mocker, caplog):
    from xplitrade.exchange.common import _reset_logging_mixin
...
async def test___async_get_candle_history_sort(default_conf, mocker, exchange_name):
    ...
    exchange = get_patched_exchange(mocker, default_conf, exchange=exchange_name)
    exchange._api_async.fetch_ohlcv = AsyncMock(return_value=ohlcv)
    sort_mock = mocker.patch("xplitrade.exchange.exchange.sorted", MagicMock(side_effect=sort_data))
    ...
    exchange._api_async.fetch_ohlcv = AsyncMock(return_value=ohlcv)
    # Reset sort mock
    sort_mock = mocker.patch("xplitrade.exchange.sorted", MagicMock(side_effect=sort_data))
    # Test the OHLCV data sort
    res = await exchange._async_get_candle_history(
        "ETH/BTC", default_conf["timeframe"], CandleType.SPOT
    )
...
def test_fetch_order(default_conf, mocker, exchange_name, caplog):
    ...
    exchange = get_patched_exchange(mocker, default_conf, api_mock, exchange=exchange_name)
    with patch("xplitrade.exchange.common.time.sleep") as tm:
        with pytest.raises(InvalidOrderException):
            exchange.fetch_order(order_id="_", pair="TKN/BTC")
