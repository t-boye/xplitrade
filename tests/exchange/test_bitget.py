...
from xplitrade.exceptions import InvalidOrderException, OperationalException, RetryableOrderError
from xplitrade.exchange.common import API_RETRY_COUNT
...
def test_fetch_stoploss_order_bitget(default_conf, mocker):
    ...
    mocker.patch("xplitrade.exchange.common.time.sleep")
    api_mock = MagicMock()
...
def test_dry_run_liquidation_price_cross_bitget(default_conf, mocker):
    ...
    with pytest.raises(
        OperationalException, match="Xplitrade currently only supports isolated futures for bitget"
    ):
