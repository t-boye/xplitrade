...
from xplitrade.configuration import (
    Configuration,
    remove_exchange_credentials,
    sanitize_config,
    validate_config_consistency,
)
from xplitrade.configuration.config_validation import validate_config_schema
from xplitrade.configuration.deprecated_settings import (
    check_conflicting_settings,
    process_deprecated_setting,
    process_removed_setting,
    process_temporary_deprecated_settings,
)
from xplitrade.configuration.environment_vars import _flat_vars_to_nested_dict
from xplitrade.configuration.load_config import (
    load_config_file,
    load_file,
    load_from_files,
    log_config_error_range,
)
from xplitrade.constants import DEFAULT_DB_DRYRUN_URL, DEFAULT_DB_PROD_URL, ENV_VAR_PREFIX
from xplitrade.enums import RunMode
from xplitrade.exceptions import ConfigurationError, OperationalException
...
def test_load_config_file(default_conf, mocker, caplog) -> None:
    ...
    file_mock = mocker.patch(
        "xplitrade.configuration.load_config.Path.open",
...
def test_load_config_file_error(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch(
        "xplitrade.configuration.load_config.Path.open", mocker.mock_open(read_data=filedata)
    )
...
def test_load_config_combine_dicts(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch("xplitrade.configuration.load_config.load_config_file", configsmock)
...
def test_print_config(default_conf, mocker, caplog) -> None:
    ...
    configsmock = MagicMock(side_effect=config_files)
    mocker.patch("xplitrade.configuration.configuration.create_datadir", lambda c, x: x)
    mocker.patch("xplitrade.configuration.configuration.load_from_files", configsmock)
...
def test_load_config_file_exception(mocker) -> None:
    mocker.patch(
        "xplitrade.configuration.configuration.Path.open",
...
def test_setup_configuration_with_arguments(mocker, default_conf, caplog, tmp_path) -> None:
    ...
    mocker.patch("xplitrade.configuration.configuration.create_datadir", lambda c, x: x)
    mocker.patch(
        "xplitrade.configuration.configuration.create_userdata_dir",
    ...
    assert log_has(f"Using user-data directory: {tmp_path / 'xplitrade'} ...", caplog)
    assert "user_data_dir" in config
...
def test_cli_verbose_with_params(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch("xplitrade.loggers.logging.config.dictConfig", MagicMock)
    arglist = ["trade", "-vvv"]
    args = Arguments(arglist).get_parsed_arg()
...
def test_pairlist_resolving_fallback(mocker, tmp_path):
    ...
    mocker.patch(
        "xplitrade.configuration.configuration.load_file",
...
def test_flat_vars_to_nested_dict(caplog):
    test_args = {
        "XPLITRADE__EXCHANGE__SOME_SETTING": "true",
        "XPLITRADE__EXCHANGE__SOME_FALSE_SETTING": "false",
        "XPLITRADE__EXCHANGE__CONFIG__whatEver": "sometime",  # Lowercased
        # Preserve case for ccxt_config
        "XPLITRADE__EXCHANGE__CCXT_CONFIG__httpsProxy": "something",
        "XPLITRADE__EXIT_PRICING__PRICE_SIDE": "bid",
        "XPLITRADE__EXIT_PRICING__cccc": "500",
        "XPLITRADE__STAKE_AMOUNT": "200.05",
        "XPLITRADE__TELEGRAM__CHAT_ID": "2151",
        "NOT_RELEVANT": "200.0",  # Will be ignored
        "XPLITRADE__ARRAY": '[{"name":"default","host":"xxx"}]',
        "XPLITRADE__EXCHANGE__PAIR_WHITELIST": '["BTC/USDT", "ETH/USDT"]',
        # Fails due to trailing comma
        "XPLITRADE__ARRAY_TRAIL_COMMA": '[{"name":"default","host":"xxx",}]',
        # Object fails
        "XPLITRADE__OBJECT": '{"name":"default","host":"xxx"}',
    }
    expected = {
        "stake_amount": 200.05,
        "exit_pricing": {
    ...
    assert log_has("Loading variable 'XPLITRADE__EXCHANGE__SOME_SETTING'", caplog)
