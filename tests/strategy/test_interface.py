...
from xplitrade.constants import CUSTOM_TAG_MAX_LENGTH
from xplitrade.data.dataprovider import DataProvider
from xplitrade.data.history import load_data
from xplitrade.enums import ExitCheckTuple, ExitType, SignalDirection
from xplitrade.exceptions import DependencyException, OperationalException, StrategyError
from xplitrade.persistence import PairLocks, Trade
from xplitrade.resolvers import StrategyResolver
from xplitrade.strategy.hyper import detect_all_parameters
...
from xplitrade.util.datetime_helpers import dt_now_no_micro
...
def test_assert_df_raise(mocker, caplog, ohlcv_history):
    ...
    mocker.patch(
        "xplitrade.strategy.interface.StrategyResultValidator.assert_df",
...
def test_advise_all_indicators_copy(mocker, default_conf, testdatadir) -> None:
    ...
    aimock = mocker.patch(
        "xplitrade.strategy.interface.IStrategy.advise_indicators",
...
def test_auto_hyperopt_interface_loadparams(default_conf, mocker, caplog):
    ...
    mocker.patch("xplitrade.strategy.hyper.HyperoptTools.load_params", return_value=expected_result)
    PairLocks.timeframe = default_conf["timeframe"]
    strategy = StrategyResolver.load_strategy(default_conf)
    ...
    mocker.patch(
        "xplitrade.strategy.hyper.HyperoptTools.load_params", MagicMock(side_effect=ValueError())
    )
