...
from xplitrade.loggers.buffering_handler import FTBufferingHandler
from xplitrade.loggers.ft_rich_handler import FtRichHandler
from xplitrade.loggers.rich_console import get_rich_console


# from xplitrade.loggers.std_err_stream_handler import FTStdErrStreamHandler
...
def _set_log_levels(
    ...
    third_party_loggers = {
        "xplitrade": logging.INFO if verbosity < 1 else logging.DEBUG,
        "xplitrade.exchange.exchange_ws": logging.INFO if verbosity <= 1 else logging.DEBUG,
...
def _create_log_config(config: Config) -> dict[str, Any]:
    ...
    for handler_config in log_config.get("handlers", {}).values():
        if handler_config.get("class") == "xplitrade.loggers.ft_rich_handler.FtRichHandler":
            ...
            try:
                ...
                raise OperationalException(
                    ...
                    "directories. If you're running xplitrade using docker, you see this error "
