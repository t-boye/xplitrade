...
from xplitrade.data.history import get_timerange, load_data
...
from xplitrade.enums import CandleType
from xplitrade.exceptions import OperationalException
from xplitrade.exchange import timeframe_to_prev_date, timeframe_to_seconds
from xplitrade.misc import pair_to_filename
...
from xplitrade.util import get_dry_run_wallet
...
def generate_profit_graph(
    ...
    fig["layout"].update(title="Xplitrade Profit plot")
    fig["layout"]["yaxis1"].update(title="Price")
...
def generate_plot_filename(pair: str, timeframe: str) -> str:
    ...
    file_name = "xplitrade-plot-" + pair_s + "-" + timeframe + ".html"
...
def plot_profit(config: Config) -> None:
    ...
    store_plot_file(
        fig,
        filename="xplitrade-profit-plot.html",
        directory=config["user_data_dir"] / "plot",
