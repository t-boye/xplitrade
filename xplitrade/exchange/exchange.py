...
from xplitrade.configuration import remove_exchange_credentials
from xplitrade.constants import (
...
from xplitrade.data.converter import (
...
from xplitrade.exchange.common import (
    API_FETCH_ORDER_RETRY_COUNT,
    retrier,
    retrier_async,
)
from xplitrade.exchange.exchange_types import (
    CcxtBalances,
    CcxtOrder,
    CcxtPosition,
    FtHas,
...
from xplitrade.exchange.exchange_ws import ExchangeWS
from xplitrade.misc import (
    chunks,
    deep_merge_dicts,
    file_dump_json,
    file_load_json,
...
from xplitrade.util import FtTTLCache, PeriodicCache, dt_from_ts, dt_now
from xplitrade.util.datetime_helpers import dt_humanize_delta, dt_ts, format_ms_time
...
class Exchange:
    ...
    def validate_trading_mode_and_margin_mode(
        ...
        """
        Checks if xplitrade can perform trades using the configured
        trading mode(Margin, Futures) and MarginMode(Cross, Isolated)
        Throws OperationalException:
            If the trading_mode/margin_mode type are not supported by xplitrade on this exchange
        """
        ...
        if (
            ...
            raise ConfigurationError(f"Xplitrade does not support '{trading_mode}' on {self.name}.")
        ...
        if not allow_none_margin_mode and (
            ...
            raise ConfigurationError(
                f"Xplitrade does not support '{mm_value}' '{trading_mode}' on {self.name}."
            )
    ...
    def fetch_dry_run_order(self, order_id: str) -> CcxtOrder:
        ...
        try:
            ...
            from xplitrade.persistence import Order
    ...
    def dry_run_liquidation_price(
        ...
        if self.trading_mode == TradingMode.FUTURES and self.margin_mode == MarginMode.ISOLATED:
            if market["inverse"]:
                raise OperationalException("Xplitrade does not yet support inverse contracts")
            ...
            raise OperationalException(
                "Xplitrade only supports isolated futures for leverage trading"
            )
