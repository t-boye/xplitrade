...
from xplitrade.configuration import TimeRange, validate_config_consistency
from xplitrade.constants import DATETIME_PRINT_FORMAT, Config, IntOrInf, LongShort
from xplitrade.data import history
from xplitrade.data.btanalysis import (
    find_existing_backtest_stats,
    get_tick_size_over_time,
    trade_list_to_dataframe,
)
from xplitrade.data.converter import trim_dataframe, trim_dataframes
from xplitrade.data.dataprovider import DataProvider
from xplitrade.data.metrics import combined_dataframes_with_rel_mean
...
from xplitrade.exchange.exchange import TICK_SIZE, Exchange
from xplitrade.ft_types import (
    BacktestContentType,
    BacktestContentTypeIcomplete,
    BacktestResultType,
    get_BacktestResultType_default,
)
from xplitrade.leverage.liquidation_price import update_liquidation_prices
from xplitrade.mixins import LoggingMixin
from xplitrade.optimize.backtest_caching import get_strategy_run_id
from xplitrade.optimize.optimize_reports import (
...
from xplitrade.persistence import (
    CustomDataWrapper,
    LocalTrade,
    Order,
    PairLocks,
...
from xplitrade.plugins.protectionmanager import ProtectionManager
from xplitrade.resolvers import ExchangeResolver, StrategyResolver
from xplitrade.strategy.interface import IStrategy
from xplitrade.strategy.strategy_wrapper import strategy_safe_wrapper
from xplitrade.util import FtPrecise, dt_now, get_progress_tracker
from xplitrade.util.migrations import migrate_data
from xplitrade.wallets import Wallets
...
if TYPE_CHECKING:
    ...
    from xplitrade.util.rich_progress import CustomProgress
...
class Backtesting:
    ...
    def __init__(
        ...
        *,
        ...
        if self.config.get("freqai_backtest_live_models", False):
            from xplitrade.freqai.utils import get_timerange_backtest_live_models
        ...
        if exit_.exit_flag:
            ...
            if exit_.exit_type in (
                ...
                if order_type == "limit":
                    ...
                    # xplitrade does not support this in live, and the order would fill immediately
                    if trade.is_short:
    ...
    def get_valid_entry_price_and_stake(
        ...
        if order_type == "limit":
            ...
            # which xplitrade does not support in live.
