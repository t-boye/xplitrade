...
from xplitrade.configuration import Configuration
from xplitrade.constants import PROCESS_THROTTLE_SECS, RETRY_TIMEOUT, Config
from xplitrade.enums import RPCMessageType, State
from xplitrade.exceptions import OperationalException, TemporaryError
from xplitrade.exchange import timeframe_to_next_date
...
class Worker:
    ...
    def _init(self, reconfig: bool) -> None:
        ...
        self.xplitrade = XplitradeBot(self._config)
    ...
    def _worker(self, old_state: State | None) -> State:
        ...
        """
        state = self.xplitrade.state

        # Log state transition
        if state != old_state:
            if old_state != State.RELOAD_CONFIG:
                self.xplitrade.notify_status(f"{state.name.lower()}")
            ...
            if state in (State.RUNNING, State.PAUSED) and old_state not in (
                ...
                self.xplitrade.startup()

            if state == State.STOPPED:
                self.xplitrade.check_for_open_trades()
        ...
        if self._heartbeat_interval:
            ...
            if (now - self._heartbeat_msg) > self._heartbeat_interval:
                version = __version__
                strategy_version = self.xplitrade.strategy.version()
    ...
    def _process_stopped(self) -> None:
        self.xplitrade.process_stopped()

    def _process_running(self) -> None:
        try:
            self.xplitrade.process()
            ...
            self.xplitrade.notify_status(
                f"*OperationalException:*\n```\n{tb}```\n {hint}", msg_type=RPCMessageType.EXCEPTION
            )

            logger.exception("OperationalException. Stopping trader ...")
            self.xplitrade.state = State.STOPPED

    def _reconfigure(self) -> None:
        """
        Cleans up current xplitradebot instance, reloads the configuration and
        replaces it with the new instance
        """
        # Tell systemd that we initiated reconfiguration
        self._notify("RELOADING=1")

        # Clean up current xplitrade modules
        self.xplitrade.cleanup()

        # Load and validate config and create new instance of the bot
        self._init(True)

        self.xplitrade.notify_status(f"{State(self.xplitrade.state)} after config reloaded")
    ...
    def exit(self) -> None:
        ...
        if self.xplitrade:
            self.xplitrade.notify_status("process died")
            self.xplitrade.cleanup()
